...
class AccountMove(models.Model):
    ...
    tax_today = fields.Float(string="Tasa", store=True,
                             default=lambda self: self.env.company.currency_id_dif.inverse_rate or 1.0,
    ...
    amount_untaxed_usd = fields.Monetary(currency_field='currency_id_dif', string="Base imponible Ref.", store=True,
                                         ...
                                         digits='Dual_Currency', copy=False)
    ...
    def create(self, values):
        ...
        if values:
            for val in values:
                if  'tax_today' not in val and not diferencia:
        ...
        moves = super().create(values)
        for move in moves:
            move._calculate_amounts_all_usd()  # Calcular manualmente después de crear
    ...
    def write(self, vals):
        res = super().write(vals)
            ...
            #self._compute_unified_amounts()
        if any(field in vals for field in ['invoice_line_ids', 'currency_id', 'tax_today', 'invoice_date', 'date']):
            #self._calculate_amounts_all_usd()  # Recalcular si cambian campos críticos
            self._compute_unified_amounts()
        return res
    ...
    def _onchange_invoice_date(self):
        if self.invoice_date:
            self._compute_date()  # Actualiza tax_today
            #self._recompute_amounts()  # Fuerza el recálculo de montos
            self._compute_unified_amounts()
            
    @api.depends('tax_today')
    def _recompute_amounts(self):
        for move in self:
            move.amount_untaxed_usd = sum(line.price_subtotal_usd for line in move.invoice_line_ids)
            move.amount_tax_usd = sum(line.price_subtotal_usd * (sum(tax.amount for tax in line.tax_ids) / 100) for line in move.invoice_line_ids)
            move.amount_total_usd = move.amount_untaxed_usd + move.amount_tax_usd
            move.amount_untaxed_bs = move.amount_untaxed_usd * move.tax_today
            move.amount_tax_bs = move.amount_tax_usd * move.tax_today
            move.amount_total_bs = move.amount_total_usd * move.tax_today
            move._verificar_pagos()
            for aml in move.line_ids:
                if aml.currency_id.id == move.currency_id_dif.id: # Solo modificar líneas en moneda extranjera
                    aml.amount_currency = aml.debit_usd if aml.debit_usd > 0 else -aml.credit_usd  # Corregir amount_currency
                    aml.debit = aml.debit_usd * move.tax_today if aml.debit_usd > 0 else 0
                    aml.credit = aml.credit_usd * move.tax_today if aml.credit_usd > 0 else 0
                elif aml.currency_id == move.company_id.currency_id: # Ajustar la contrapartida en moneda local
                    aml.price_unit = aml.price_unit_usd * move.tax_today 
                    aml.price_subtotal = aml.price_subtotal_usd * move.tax_today 
                    if aml.account_id == move.journal_id.default_account_id:
                        aml.debit = move.amount_total if move.amount_total > 0 else 0
                        aml.credit = -move.amount_total if move.amount_total < 0 else 0
                        
                        
    # Método para calcular los montos (ya no es computado)
    def _calculate_amounts_all_usd(self):
        for rec in self:
            if rec.is_invoice(include_receipts=True) and rec.tax_totals:
                ...
                amount_total = rec.tax_totals['amount_total'] if rec.tax_totals['amount_total'] else 0
                if rec.currency_id != self.env.company.currency_id:
                    rec.amount_untaxed_usd = rec.amount_untaxed if rec.amount_untaxed else 0
                    rec.amount_tax_usd = rec.amount_tax if rec.amount_tax else 0
                    rec.amount_total_usd = rec.amount_total if rec.amount_total else 0
    ...
    @api.depends('invoice_line_ids', 'invoice_line_ids.price_subtotal', 'invoice_line_ids.tax_ids', 'tax_today', 'currency_id')
    def _compute_unified_amounts(self):
        for move in self:
            if move.is_invoice(include_receipts=True):
                
                # 1. Calcular montos en USD
                ...
                for line in move.invoice_line_ids:
                    ...
                    if move.currency_id == move.company_id.currency_id:
                        price_subtotal_usd = line.price_subtotal / move.tax_today if move.tax_today > 0 else 0
                    else:
                        price_subtotal_usd = line.price_subtotal

                    amount_untaxed_usd += price_subtotal_usd

                    # Impuestos en USD (usar el cálculo real de impuestos, no solo el %)
                    tax_lines = line.tax_ids.compute_all(
                    ...
                    for tax in tax_lines['taxes']:
                        ...
                        if move.currency_id == move.company_id.currency_id:
                            tax_amount_usd = tax_amount / move.tax_today if move.tax_today > 0 else 0
                        else:
                            tax_amount_usd = tax_amount
                        amount_tax_usd += tax_amount_usd

                # Asignar valores
                move.amount_untaxed_usd = amount_untaxed_usd
                move.amount_tax_usd = amount_tax_usd
                move.amount_total_usd = amount_untaxed_usd + amount_tax_usd

                # 2. Calcular montos en BS
                move.amount_untaxed_bs = amount_untaxed_usd * move.tax_today
                move.amount_tax_bs = amount_tax_usd * move.tax_today
                move.amount_total_bs = move.amount_total_usd * move.tax_today

                # 3. Actualizar líneas contables
                move._update_account_move_lines()

    def _update_account_move_lines(self):
        for move in self:
            for aml in move.line_ids:
                # Calcular valores en USD basados en el valor BS y la tasa
                if move.currency_id == move.company_id.currency_id:  # Moneda BS
                    if move.tax_today > 0:
                        aml.price_unit_usd = aml.price_unit / move.tax_today
                        aml.price_subtotal_usd = aml.price_subtotal / move.tax_today
                    else:
                        aml.price_unit_usd = 0.0
                        aml.price_subtotal_usd = 0.0
                else:  # Moneda USD
                    aml.price_unit_usd = aml.price_unit
                    aml.price_subtotal_usd = aml.price_subtotal
                    
                aml._debit_usd()
                aml._credit_usd()

                # Actualizar débitos/créditos según moneda
                #if aml.currency_id == move.company_id.currency_id:
                    # Líneas en moneda local (BS)
                #    if aml.account_id == move.journal_id.default_account_id:
                #        aml.debit = move.amount_total_bs if move.amount_total_bs > 0 else 0
                #        aml.credit = -move.amount_total_bs if move.amount_total_bs < 0 else 0
                #        print('Aqui')
                #else:
                #    print(move.amount_total_usd)
                #    aml.debit = aml.debit_usd * move.tax_today if aml.debit_usd else 0
                #    aml.credit = aml.credit_usd * move.tax_today if aml.credit_usd else 0
    ...
    def _recompute_dynamic_lines(self, recompute_all_taxes=False, recompute_tax_base_amount=False):
        ...
        self._compute_unified_amounts()  # Forzar recálculo en ambas monedas
        return res
