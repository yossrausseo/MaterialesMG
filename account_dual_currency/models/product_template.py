...
class Productos(models.Model):
    ...
    @api.model
    def update_list_price_usd_in_variants(self, values):
        if 'list_price_usd' in values:
            for product in self.product_variant_ids:
                product.write({'list_price_usd': values.get('list_price_usd')})

    @api.model
    def create(self, values):
        record = super(Productos, self).create(values)
        record.update_list_price_usd_in_variants(values)
        return record

    @api.model
    def write(self, values):
        if 'list_price_usd' in values:
            self.update_list_price_usd_in_variants(values)
        return super(Productos, self).write(values)

    def send_update_to_variants(self, values):
        product_product_model = self.env['product.product']
        product_product_model.update_list_price_usd_in_variants(self.product_variant_ids, values.get('list_price_usd'))
