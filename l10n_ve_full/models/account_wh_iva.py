...
class AccountWhIva(models.Model):
    ...
    def action_send_mail(self):
        for rec in self:
            if rec.state != 'done':
                raise UserError("No se puede enviar el correo electrónico, el documento no está confirmado")
            template_id = self.env.ref('l10n_ve_full.email_template_for_retencion_iva').id
            template = self.env['mail.template'].browse(template_id)
            mail_id = template.send_mail(self.id, force_send=True)
            rec.message_post(body=_(u'Correo electrónico enviado con éxito'))
