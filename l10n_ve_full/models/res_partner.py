...
class ResPartner(models.Model):
    ...
    agente_retencion_mun = fields.Boolean('¿Es Agente de Retención Municipal?')
    licencia_municipal = fields.Char('Licencia Municipal')
    municipal_rate_id = fields.Many2many('account.wh.municipal.rates', string='Conceptos de retención municipal')
    porc_reten_muni = fields.Float('Porcentaje de Retención Municipal', digits=dp.get_precision('Account'), default=100.0)

    journal_muni_purchase_id = fields.Many2one('account.journal', 'Diario de Compra Ret. Municipal', company_dependent=True,
                                        domain="[('company_id', '=', current_company_id)]", default=lambda self: self.env.company.journal_muni_purchase_id)
    journal_muni_sale_id = fields.Many2one('account.journal', 'Diario de Venta Ret. Municipal', company_dependent=True,
                                        domain="[('company_id', '=', current_company_id)]", default=lambda self: self.env.company.journal_muni_sale_id)
    ...
    def default_get(self, default_fields):
        ...
        country_id = company_id.partner_id.country_id
        if country_id:
            res.update({'country_id': country_id.id})
        state_id = company_id.partner_id.state_id
        if state_id:
