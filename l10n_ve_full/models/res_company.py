...
class ResCompany(models.Model):
    ...
    #retención municipal
    aplicar_retencion_municipal = fields.Boolean(
        'Retención Municipal',
        help='Cuando sea Verdadero, la Retención Municipal estará disponible en el pago de factura',
        default=False)
    aplicar_sobre = fields.Selection([
        ('amount_untaxed', 'Monto sin impuesto'),
        ('amount_total', 'Monto total'),
    ], string='Aplicar sobre', default='amount_untaxed', required=True)

    account_debit_wh_municipal_id = fields.Many2one('account.account', string="Cuenta Recibos Municipal",
                                                    help="Esta cuenta se utilizará en lugar de la predeterminada"
                                                         "para generar el asiento de la retención municipal")
    account_credit_wh_municipal_id = fields.Many2one('account.account', string="Cuenta Pagos Municipal",
                                                        help="Esta cuenta se utilizará en lugar de la predeterminada"
                                                            "para generar el asiento de la retención municipal")

    journal_muni_purchase_id = fields.Many2one('account.journal', string='Diario de Compras Municipal',
                                                help="Diario para la retención municipal de compras")
    journal_muni_sale_id = fields.Many2one('account.journal', string='Diario de Ventas Municipal',
                                            help="Diario para la retención municipal de ventas")
