...
class ResConfigSettings(models.TransientModel):
    ...
    #retencion municipal
    aplicar_retencion_municipal = fields.Boolean(related="company_id.aplicar_retencion_municipal", readonly=False)
    aplicar_sobre = fields.Selection(related="company_id.aplicar_sobre", readonly=False)
    account_debit_wh_municipal_id = fields.Many2one('account.account', string="Cuenta Recibos Retención Municipal", related="company_id.account_debit_wh_municipal_id", readonly=False)
    account_credit_wh_municipal_id = fields.Many2one('account.account', string="Cuenta Pagos Retención Municipal", related="company_id.account_credit_wh_municipal_id", readonly=False)
    journal_muni_purchase_id = fields.Many2one('account.journal', string="Diario Retención Municipal Compra", related="company_id.journal_muni_purchase_id", readonly=False)
    journal_muni_sale_id = fields.Many2one('account.journal', string="Diario Retención Municipal Venta", related="company_id.journal_muni_sale_id", readonly=False)
